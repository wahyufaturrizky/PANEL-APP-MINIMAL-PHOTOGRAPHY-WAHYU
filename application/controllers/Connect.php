...
class Connect extends CI_Controller {
	...
	private $tbl = 'dtb_wahyudin_connect';
	...
	public function index(){
		...
		$data = [
			...
			'content' => $this->load->view('back/connect/index' ,'',true),
		];
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img_01 = "<img src='".base_url()."assets/picture/pic_connect/".$val->pic_01."' style='height:80px;width:80px;'>";
            		$img_02 = "<img src='".base_url()."assets/picture/pic_connect/".$val->pic_02."' style='height:80px;width:80px;'>";
            		$img_03 = "<img src='".base_url()."assets/picture/pic_connect/".$val->pic_03."' style='height:80px;width:80px;'>";
            		$img_04 = "<img src='".base_url()."assets/picture/pic_connect/".$val->pic_04."' style='height:80px;width:80px;'>";
	...
	function save_connect($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/pic_connect/';
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/pic_connect/'.$old_img;
