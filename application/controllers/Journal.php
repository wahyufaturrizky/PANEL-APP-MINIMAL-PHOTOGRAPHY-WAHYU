...
class Journal extends CI_Controller {
	...
	private $tbl = 'dtb_wahyudin_journal';
	...
	public function index(){
		...
		$data = [
			...
			'content' => $this->load->view('back/journal/index' ,'',true),
		];
	...
	function get_data_journal(){
		...
		$where_like['data'][] = array(
			'column' => 'id,picture,header,description,create_date,udpate_date,status',
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/pic_journal/".$val->picture."' style='height:52px;width:52px;'>";
	...
	function save_journal($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/pic_journal/';
		...
		$data = [
			...
			'udpate_date' => $this->input->post('udpate_date'),
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/pic_journal/'.$old_img;
